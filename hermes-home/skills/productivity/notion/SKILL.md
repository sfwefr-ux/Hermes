...
## Path C — Notion MCP через MCP Market (альтернатива)

Notion MCP (v1.2.0+) доступен через MCP Market по тому же OAuth-токену, что и Miro:
...
mcp_servers:
  mcpmarket_notion:
    ...
    headers:
      ...
      Accept: 'application/json, text/event-stream'  # ОБЯЗАТЕЛЬНО для SSE
    timeout: 120
    connect_timeout: 60
```

**Особенности:**
- Тот же Supabase JWT-токен, что для Miro — отдельный ключ не нужен
- Требует заголовок `Accept: application/json, text/event-stream` (без него — 406)
- Токен — JWT, НЕ `sk_user_...` (тот только для аккаунта)
- После добавления в config — `/restart` или перезапуск Hermes
- Инструменты: notion_search, notion_fetch, notion_create_pages, notion_update_page, notion_create_database, notion_create_view, notion_get_comments, notion_get_users и др.

**Преимущества перед curl/ntn:**
- Не нужен `NOTION_API_KEY` в .env
- Автоматическая пагинация
- Markdown из коробки (fetch возвращает enhanced markdown)
- Работает из коробки после подключения MCP-сервера
- Кириллица работает идеально (в отличие от Miro DSL)
- См. [references/mcp-workflows.md](references/mcp-workflows.md) — шаблоны DDL, создания записей, cron-аналитики

## Choosing the Right Path

| Task | mac / Linux | Windows | MCP Market |
|---|---|---|---|---|
| Read/write pages, search, query databases | `ntn api ...` | curl | notion_* tools |
| Read a page for an agent to summarize | `ntn api v1/pages/{id}/markdown` | curl `/markdown` endpoint | `notion_fetch` (auto markdown) |
| Upload a file | `ntn files create < file` | 3-step HTTP flow | N/A |
| One-off API exploration | `ntn api ...` | curl | `notion_search` |
| Build a sync / webhook / agent tool hosted by Notion | `ntn workers ...` | WSL2 + `ntn workers ...` | N/A |
| **Zero-config setup (no API key)** | ❌ | ❌ | ✅ (reuses Miro MCP token) |
| **Cyrillic content** | ✅ | ✅ | ✅ (perfect) |

## Pitfalls

### update_content requires EXACT text matching
When using `notion_update_page` with `command=update_content`, the `old_str` in each `content_updates` entry MUST match the page content exactly — character-for-character, including whitespace and newlines. The Notion API returns "No matches found" on any mismatch. Workflow: always call `notion_fetch` first to get the current raw text, copy the exact substring you want to replace, then use it in `old_str`. Prefer short, unique fragments (one sentence or heading) over multi-line blocks to reduce mismatch risk.

### MCP server may become temporarily unreachable
After 3+ consecutive failures the MCP server auto-pauses for ~60 seconds. Wait and retry with a different approach (e.g., shorter old_str, different tool).

## Notes

- **update_content pitfall:** The `old_str` parameter requires EXACT match of the page content, including whitespace and newlines. If the page was created or last edited by a different tool path (e.g. `notion_create_pages` vs direct API), the stored Markdown may differ from what `notion_fetch` returns. When `update_content` fails with "No matches found", use `notion_fetch` first, copy the exact snippet from the response, and retry. If it still fails, use `replace_content` (rewrites entire page) as fallback — but warn the user first since it's destructive.
...
- **MCP vs Direct API comparison:** see [`references/notion-mcp-vs-api.md`](references/notion-mcp-vs-api.md) — когда какой способ использовать, полный список MCP-инструментов, формат Markdown.
- **Small business finance tracking pattern:** see [`references/badlock-finance-tracking.md`](references/badlock-finance-tracking.md) — database schema, cron analytics, voice-entry workflow.

## MCP Pitfalls

### `update_content`: old_string must match EXACTLY
Строгая замена — пробелы, переводы строк и спецсимволы должны совпадать побайтово. Всегда делать `notion_fetch` перед `update_content`, копировать точный текст. После 3 ошибок подряд MCP-сервер уходит в недоступность на ~60 секунд.

### `update_data_source` ALTER COLUMN: перечислить ВСЕ опции
При `ALTER COLUMN "Имя" SET SELECT(...)` нужно включать все существующие опции + новые. Инструмент заменяет список целиком, а не добавляет. Пропущенная опция удаляется.

### Кириллица в MCP
Работает идеально через Notion MCP (в отличие от Miro DSL). Смело использовать русский текст везде.
