...
Если пользователь присылает ДВА (или больше) фото:
- **Фото 1** — его объект (мебель, предмет) → база для img2img
- **Фото 2** — референс интерьера (как должно выглядеть) → только для описания в промпте

❌ НЕ делай img2img на референсе интерьера. Начинай с ФОТО 1 (объект пользователя).
✅ Промпт = точное описание объекта с Фото 1 + описание интерьера с Фото 2.

Если пользователь присылает одно фото — это его объект, img2img на нём.
